...
function makeBranch(depth) {
  ...
  var height = Math.random()*9+1;
  branch.style.marginBottom = -height + "em";
  branch.style.height = height + "em";

  var angle = Math.random()*(60*(depth+1)) - (30*(depth+1));
  branch.style.transform = 'rotate3d(' + (Math.random()>0.5?1:0) + "," + (Math.random()>0.5?1:0) + "," + (Math.random()>0.5?1:0) + "," + angle + "deg)";

  if (depth < 4) {
    var children = Math.round(Math.random()*1)+2;
    for (var i=0; i<children; i++) {
      branch.appendChild(makeBranch(depth+1));
    }
  } else {
    var leaf = makeLeaf();
...
document.body.addEventListener("mousemove", function(e) {
	...
	world.style.transform = "rotateX("+m.rx+"deg) rotateY("+m.ry+"deg) rotateZ(180deg)";
});
